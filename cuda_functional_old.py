# from commit 711b7747b77dc75b04cc5f2467cdf544d1b92f8d 
...
def SRU_Compute_CPU(activation_type, d, bidirectional=False):
    ...
    def sru_compute_cpu(u, x, bias, init=None, mask_h=None):
        ...
        return h.view(length, batch, -1), torch.cat(c_final, dim=1)

    return sru_compute_cpu

def SRU_Compute_CPU_cy(activation_type, d, bidirectional=False):
    """CPU version of the core SRU computation.
    Has the same interface as SRU_Compute_GPU() but is a regular Python function
    instead of a torch.autograd.Function because we don't implement backward()
    explicitly.
    ...
    def sru_compute_cpu(u, x, bias, init=None, mask_h=None):
        # don't care Variable
        u, x, bias = u.data, x.data, bias.data
        ...
        if init is None:
            c_init = x.data.new(batch, bidir * d).zero_()
        else:
            c_init = init.data.view(batch, bidir * d)

        if bidirectional:
            h, c_f = sru_compute_bi_cpu_cy(x_prime[:, :, :, :].numpy(),
                                           x_tilde[:, :, :, :].numpy(),
                                           forget[:, :, :, :].numpy(),
                                           reset[:, :, :, :].numpy(),
                                           c_init.numpy(),
            ...
            h, c_f = sru_compute_cpu_cy(x_prime[:, :, 0, :].numpy(),
                                        x_tilde[:, :, 0, :].numpy(),
                                        forget[:, :, 0, :].numpy(),
                                        reset[:, :, 0, :].numpy(),
                                        c_init.numpy(),
        ...
        c_f = torch.from_numpy(c_f)
        return Variable(h, volatile=True), Variable(c_f, volatile=True)
        # return Variable(torch.cat(h_all, 2), volatile=True), Variable(torch.cat(c_final, 1), volatile=True)
...
class SRUCell(nn.Module):
    ...
    def forward(self, input, c0=None):
        ...
        # print('preprocess time: {:.4f} ms'.format((time.time() - start) * 1000))
        # start = time.time()
        u = x_2d.mm(self.weight)
        # print('maxtrix multiplication time: {:.4f} ms'.format((time.time() - start) * 1000))
        # start = time.time()

        if input.is_cuda:
            SRU_Compute = SRU_Compute_GPU(self.activation_type, n_out, self.bidirectional)
        elif self.cy:
            SRU_Compute = SRU_Compute_CPU_cy(self.activation_type, n_out, self.bidirectional)
        else:
        ...
        if self.training and (self.dropout>0):
            ...
            output = SRU_Compute(u, input, self.bias, c0)
            # print('sequential part time: {:.4f} ms'.format((time.time() - start) * 1000))
            return output
...
class GroupSRUCell(nn.Module):
    def __init__(self, n_in, n_out, dropout=0, rnn_dropout=0,
                 groups=4, bidirectional=False, use_tanh=1, use_relu=0):
        # super(GroupSRUCell, self).__init__()
        super().__init__()
        n_in = int(np.ceil(n_in/groups)) * groups
        self.n_in = n_in
        self.n_out = n_out
        self.rnn_dropout = rnn_dropout
        self.dropout = dropout
        ...
        self.weight = nn.Parameter(torch.Tensor(
            self.groups,
            n_in // self.groups,
            (self.size_per_dir*2 if bidirectional else self.size_per_dir) // self.groups
        ))
        self.bias = nn.Parameter(torch.Tensor(
            n_out*4 if bidirectional else n_out*2
        ))
    ...
    def forward(self, input, c0=None):
        ...
        # start = time.time()
        n_in, n_out = self.n_in, self.n_out
        if input.size(2) < n_in:
            zeros = Variable(input.data.new(*input.size()[:2], n_in - input.size(2)).zero_(), requires_grad=False)
        ...
        x_2d = x if x.dim() == 2 else x.contiguous().view(-1, n_in)
        x_g2d = x_2d.view(-1, self.groups, n_in // self.groups).transpose(0, 1)
        # print('preprocess time: {:.4f} ms'.format((time.time() - start) * 1000))
        # start = time.time()
        u_g = x_g2d.bmm(self.weight)
        u = u_g.transpose(0, 1).contiguous().view(-1, self.weight.size(2) * self.groups)
        # print('maxtrix multiplication time: {:.4f} ms'.format((time.time() - start) * 1000))
        # start = time.time()
